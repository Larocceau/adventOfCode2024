...
let text = fileName |> System.IO.File.ReadAllText
...
let solution1 = mults |> Seq.map snd |> Seq.sum

printfn $"the first solution is: {solution1}"

let dos = text |> Regex.matches doRegex |> Seq.map (_.Index)

let donts = text |> Regex.matches dontRegex |> Seq.map (_.Index)
...
|> List.fold
    (fun (counting, total) instruction ->
        ...
        | Do -> (true, total)
        | Dont -> (false, total)
        | Mult value -> (counting, (if counting then total + value else total))
    )
    (true, 0)